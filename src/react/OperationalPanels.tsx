...
export function BackupsPanel({ adapter, title = "Backups", runLabel = "Run backup", pageSize = 25, className, formatTimestamp }: BackupsPanelProps) {
  ...
  return <section className={["admin-kit__operations", className].filter(Boolean).join(" ")} aria-label={title}><header><div><h2>{title}</h2><p>{result.total} recent recovery points</p></div>{adapter.run ? <button className="admin-kit__button admin-kit__button--primary" disabled={busy} type="button" onClick={() => void run()}>{runLabel}</button> : null}</header>{error ? <AdminPanelStateView state={{ kind: "error", detail: error, onRetry: () => void load() }} /> : null}<div className="admin-kit__table-wrap admin-kit__operations-table-wrap"><table className="admin-kit__table admin-kit__operations-table" aria-busy={busy}><thead><tr><th scope="col">Backup</th><th scope="col">Created</th><th scope="col">Size</th><th scope="col">Status</th>{adapter.restore ? <th scope="col">Actions</th> : null}</tr></thead><tbody>{result.items.length === 0 ? <tr><td className="admin-kit__operations-empty" colSpan={columnCount}>No backups yet. Run a backup to create your first recovery point.</td></tr> : result.items.map(item => <tr key={item.id}><td><strong>{item.label}</strong>{item.detail ? <small>{item.detail}</small> : null}</td><td>{formatAdminTimestamp(item.createdAt, formatTimestamp)}</td><td>{item.size ?? "—"}</td><td><span className={`admin-kit__state-pill admin-kit__state-pill--${item.state}`}>{item.state}</span></td>{adapter.restore ? <td><button disabled={busy || item.state !== "completed"} type="button" onClick={() => setRestoreTarget(item)}>Restore</button></td> : null}</tr>)}</tbody></table></div>{totalPages > 1 ? (
