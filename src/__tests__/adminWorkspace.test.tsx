...
describe("AdminWorkspace", () => {
  it("provides semantic page framing and optional operational slots", () => {
    render(
      <AdminWorkspace
        ...
        actions={<button>Run backup</button>}
        summary={<span>Healthy</span>}
        toolbar={<input aria-label="Search backups" />}
      >
        Records
      </AdminWorkspace>,
    );
    expect(screen.getByRole("main").textContent).toContain("Backups");
    expect(screen.getByRole("region", { name: "Backups summary" }).textContent).toContain(
      "Healthy",
    );
  ...
  it("can compose inside a host-owned main landmark", () => {
    render(
      <main>
        <AdminWorkspace as="section" title="Backups">
          Records
        </AdminWorkspace>
      </main>,
    );
  ...
  it("omits the visible workspace title band when a nested panel owns the heading", () => {
    const { container } = render(
      <AdminWorkspace showHeader={false} title="Users">
        <h2>Users</h2>
      </AdminWorkspace>,
    );
  ...
  it("leaves a bare content boundary for a panel-led page header", () => {
    const { container } = render(
      <AdminWorkspace presentation="panel-led" title="Users">
        <h1>Users</h1>
      </AdminWorkspace>,
    );
