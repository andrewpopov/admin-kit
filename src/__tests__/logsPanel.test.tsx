...
const snapshot = {
  source: "app",
  sources: [{ value: "app", label: "Application", detail: "app.log" }, { value: "pm2", label: "PM2" }],
  entries: [
    { id: "1", timestamp: "2026-07-14T20:00:00.000Z", level: { value: "info", label: "INFO", tone: "info" as const }, category: "http", message: "Request completed", raw: "INFO Request completed" },
    { id: "2", timestamp: "2026-07-14T20:01:00.000Z", level: { value: "error", label: "ERROR", tone: "danger" as const }, category: "worker", message: "Worker failed", raw: "ERROR Worker failed" },
  ],
  total: 2,
  levels: [{ value: "info", label: "Info" }, { value: "error", label: "Error" }],
  categories: [{ value: "http", label: "HTTP" }, { value: "worker", label: "Worker" }],
};

function adapter(read = vi.fn().mockResolvedValue(snapshot)) {
  return defineAdminLogsAdapter({ read, defaultSource: "app", lineLimits: [100, 200], defaultLineLimit: 200 });
}
...
describe("runtime log contracts", () => {
  ...
  it("rejects undeclared sources and levels before rendering", () => {
    expect(() => validateAdminLogsSnapshot({ ...snapshot, source: "missing" })).toThrow(/not declared/i);
    expect(() => validateAdminLogsSnapshot({ ...snapshot, entries: [{ ...snapshot.entries[0]!, level: { value: "fatal", label: "Fatal" } }] })).toThrow(/undeclared level/i);
  });
  ...
  it("represents a host with no runtime log sources as an empty snapshot", () => {
    ...
    expect(() => validateAdminLogsSnapshot({ source: "missing", sources: [], entries: [], total: 0 })).toThrow(/without sources/i);
  });

  it("rejects invalid line-limit configuration", () => {
    expect(() => defineAdminLogsAdapter({ read: vi.fn(), lineLimits: [200, 200] })).toThrow(/unique/i);
    expect(() => defineAdminLogsAdapter({ read: vi.fn(), lineLimits: [100], defaultLineLimit: 200 })).toThrow(/must be declared/i);
  });
});
...
describe("LogsPanel", () => {
  it("renders normalized sources, filters, bounded lines, and console output", async () => {
    ...
    expect(read).toHaveBeenCalledWith({ source: "app", limit: 200, level: undefined, category: undefined, search: undefined });
  });
  ...
  it("filters the loaded snapshot immediately and applies search to the adapter", async () => {
    ...
    fireEvent.change(screen.getByRole("searchbox", { name: "Search logs" }), { target: { value: "worker" } });
    expect(screen.queryByText("Request completed")).toBeNull();
    expect(screen.getByText("Worker failed")).toBeTruthy();
    fireEvent.click(screen.getByRole("button", { name: "Apply" }));
    await waitFor(() => expect(read).toHaveBeenLastCalledWith(expect.objectContaining({ search: "worker" })));
  });

  it("keeps the last snapshot visible when refresh fails", async () => {
    const read = vi.fn().mockResolvedValueOnce(snapshot).mockRejectedValueOnce(new Error("Log host unavailable"));
    render(<LogsPanel adapter={adapter(read)} />);
    await screen.findByText("Request completed");

    fireEvent.click(screen.getByRole("button", { name: "Refresh" }));
    expect(await screen.findByText("Log host unavailable")).toBeTruthy();
    expect(screen.getByText("Refresh failed. The previously loaded output remains available.")).toBeTruthy();
  ...
  it("starts in tail-follow mode and exposes a named keyboard scroll region", async () => {
    ...
    expect(screen.getByRole("switch", { name: "Follow latest on" }).getAttribute("aria-checked")).toBe("true");
    fireEvent.click(screen.getByRole("switch", { name: "Follow latest on" }));
    expect(screen.getByRole("switch", { name: "Follow latest off" }).getAttribute("aria-checked")).toBe("false");
  });
  ...
  it("ignores a stale source response that resolves after the current source", async () => {
    ...
    const read = vi.fn()
      ...
      .mockImplementationOnce(() => new Promise((resolve) => { resolvePm2 = resolve; }))
      .mockResolvedValueOnce({ ...snapshot, entries: [{ ...snapshot.entries[0]!, id: "current", message: "Current app line" }] });
    render(<LogsPanel adapter={adapter(read)} />);
    await screen.findByText("Request completed");

    fireEvent.change(screen.getByRole("combobox", { name: "Source" }), { target: { value: "pm2" } });
    await waitFor(() => expect(read).toHaveBeenCalledTimes(2));
    fireEvent.change(screen.getByRole("combobox", { name: "Source" }), { target: { value: "app" } });
    await screen.findByText("Current app line");
    resolvePm2?.({ ...snapshot, source: "pm2", entries: [{ ...snapshot.entries[0]!, id: "stale", message: "Stale PM2 line" }] });
  ...
  it("exposes optional polling as an accessible user-controlled switch", async () => {
    ...
    expect(screen.getByRole("switch", { name: "Auto-refresh on" }).getAttribute("aria-checked")).toBe("true");
  });
