...
function validateValues(name: string, values: readonly AdminUserValue[] | undefined): void {
  ...
  for (const value of values) {
    if (!value.value.trim()) throw new Error(`${name} values must not be empty.`);
    if (!value.label.trim()) throw new Error(`${name} value ${value.value} needs a label.`);
    if (seen.has(value.value)) throw new Error(`Duplicate ${name} value: ${value.value}.`);
