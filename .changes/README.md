...
`breaking`, `added`, `changed`, `fixed`, `security`. This package's
`CHANGELOG.md` groups entries under a `### <Heading>` subsection per kind
(`### Added`, `### Changed`, `### Fixed`, ...), so the kind also controls
which subsection a fragment's bullet lands in.
